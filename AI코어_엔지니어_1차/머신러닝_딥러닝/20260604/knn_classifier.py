...
# length 와 weight 이 두가지 특성데이터를 모델 학습에 입력데이터(train_x)로 활용
...
# 모델 준비
knnmodel = KNeighborsClassifier() # k=5 가 디폴트 설정됨

# fish data 산점도 시각화
plt.scatter(bream_length, bream_weight) # bream 데이터 시각화
plt.scatter(smelt_length, smelt_weight) # smelt 데이터시각화
...
# # 모델 예측
# # 1 : bream , 0 : smelt
# print( knnmodel.predict([[30, 420],[9,12]])) # predict()함수는 하나의 데이터 전달이어도 2차원배열로 전달