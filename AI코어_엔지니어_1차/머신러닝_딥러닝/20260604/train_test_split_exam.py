...
bream_weight = [242.0, 290.0, 340.0, 363.0, 430.0, 450.0, 500.0, 
                390.0, 450.0, 500.0, 475.0, 500.0,500.0, 340.0, 600.0, 
                600.0, 700.0, 700.0, 610.0, 650.0, 575.0, 685.0, 620.0, 680.0,
...
print(fish_data[:5])
print(len(fish_data))  # 49 개

# np.concatenate : 두 넘파이 배열을 병합해라
# arr = np.ones( (2,3) )  # (2,3) 넘파이 배열을 만들고 각 요소를 모두 1로 채워
# print(arr)
# arr = np.zeros( (2,3) )  # (2,3) 넘파이 배열을 만들고 각 요소를 모두 0로 채워
# print(arr)

# arr = np.ones( 35 )  # 길이가 35인 1차원 넘파이 배열을 만들고 각 요소를 모두 1로 채워
# print(arr)
# arr = np.zeros( 14 )  # 길이가 14인 1차원 넘파이 배열을 만들고 각 요소를 모두 0로 채워
# print(arr)
fish_target = np.concatenate( ( np.ones(35), np.zeros(14) ) )
print(fish_target) # 49개 

print(fish_data[:5])
# 원 데이터를 train / test로  분할 ( 기본 suffle 제공 ) 
...
# [25,150] 데이터 기준 거리가 가까운 k개(5개)의 데이터 index를 반환
...
# plt.scatter( train_x[index, 0], train_x[index,1] , marker='^' , c='red' )

# plt.scatter(train_x[ : , 0] , train_x[ : , 1])
# plt.scatter(25, 150, marker='^', c='blue')
