...
embedding_dim = 100  # embedding 밀집 벡터 차원
hidden_units = 128 # LSTM 뉴런 수
model = Sequential()
...
optimizer = tf.keras.optimizers.Adam(learning_rate = 1e-5) # 0.0001
model.compile(loss = 'binary_crossentropy', optimizer = optimizer, metrics=['accuracy'])

# 모델 Callbacks 지정
EarlyStopCB = EarlyStopping(monitor='val_loss', verbose=1, patience=4, 
                            restore_best_weights=True)
ModelCheckCB = ModelCheckpoint('movie_review_bestmodel.keras', monitor='loss',
                               verbose=1, save_best_only=True)

# 학습

record = model.fit(X_train_pades, y_train,
                   validation_data = (X_test_pades, y_test), epochs=70, 
                   callbacks=[EarlyStopCB, ModelCheckCB], batch_size=64)